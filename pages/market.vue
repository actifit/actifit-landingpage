			...
			<div class="market-toolbar mb-4" v-if="prodList.length">
				<div class="market-toolbar-row">
					...
					<div class="toolbar-controls">
						...
						<button type="button" class="filter-trigger" @click="filterDrawerOpen = true">
					...
					<aside class="filter-drawer" role="dialog" aria-modal="true" :aria-label="$t('Filter_By')">
						...
						<div class="filter-drawer-body">
							<fieldset class="filter-section">
								...
								<button type="button" v-for="opt in filterOptions" :key="opt.value" class="drawer-filter-option"
				...
				<aside class="market-sidebar" :class="{ 'mobile-hidden': mobileShowDetail }"
					...
					<div class="sidebar-scroll">
						<div class="sidebar-group" :class="{
							'sidebar-group-owned': group.type === 'owned',
							...
							<button type="button" class="sidebar-row" v-for="product in group.items" :key="product._id"
								...
								<span class="sidebar-row-side">
									<span class="sidebar-row-owned" v-if="group.type === 'owned'">
...
import { filterMarketProducts, getGadgetOwnership, getOwnedGadgetProducts } from '~/utils/marketCatalog'
...
export default {
	...
	computed: {
		...
		groupedProducts() {
			...
			const ownedGadgets = getOwnedGadgetProducts(this.filteredProducts, this.gadgetStats);
			const personalGroups = [];
			if (savedProducts.length > 0) {
				personalGroups.push({ type: 'saved', labelKey: 'saved_products', items: savedProducts });
			}
			if (this.user && ownedGadgets.length > 0) {
				personalGroups.push({ type: 'owned', labelKey: 'my_gadgets', items: ownedGadgets });
			}
	...
	methods: {
		...
		statusDot(product) {
			...
			if (product.type == 'ingame' && Array.isArray(this.gadgetStats)) {
				const ownership = getGadgetOwnership(this.gadgetStats, product._id);
