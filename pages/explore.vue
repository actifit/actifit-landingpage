	...
	<div>
		...
		<div class="container pt-5 mt-5 pb-2">
			...
			<div v-if="popComms && popComms.length > 0" class="row comm-container col-12" ref="comm-container">
				<span v-for="(selComm) in popComms" :key="selComm[0] + 'chk'" class="comm-selector col-3">
					<input type="checkbox" :ref="selComm[0] + 'chk'" :id="selComm[0] + 'chk'"
						@change="($event) => handleCheckboxChange($event, selComm[0], selComm[1])">
					<!--<a :href="selComm[0]" class="text-brand" >-->

					<label :for="selComm[0] + 'chk'" :value="selComm[1]"><img :src="$getCommunityLogo(selComm[0])"
							:alt="selComm[1]" class="community-image">{{ selComm[1] }}</label>
					<!--</a>-->
				</span>
			</div>
			<div v-else>
				<i class="fas fa-spinner fa-spin fa-3x text-brand"
					style="display:flex; justify-content: center;text-align: center; margin-bottom: 3rem; margin-top: 2rem;"></i>
			</div>
			<span v-if="popComms && popComms.length > 0" class="expander"><i ref="switcher"
					title="Show More Communities" class="fas fa-solid fa-arrow-circle-down text-brand"
					@click="expandComms"></i></span>
        ...
        <i class="fas fa-spinner fa-spin text-brand"></i>
			...
			<div class="text-center p-3" v-if="user && user.account.name == username">
				<a href="#" @click="initiateNewPost" data-toggle="modal" data-target="#editPostModal"
					:title="$t('Create_post')" class="btn btn-brand border">
					{{ $t('Create_post') }}
				</a>
			</div>
...
export default {
	...
	async mounted() {
		...
		// Grab popular communities and assign directly to this.popComms
		this.popComms = await this.$store.dispatch('fetchPopCommunities');

		// --- START OF THE FIX ---
		// After fetching, immediately check if the result is a valid array.
		// If not (e.g., the API failed), set it to an empty array to prevent all future errors.
		if (!Array.isArray(this.popComms)) {
			this.popComms = [];
		}
		// --- END OF THE FIX ---

		// Now, we can safely work with this.popComms
		...
		// Reset and re-populate the array. This will trigger the v-for in the template.
		this.popComms = Array.from(combinedSet).map(JSON.parse);

		// Set the initial selected communities for rendering the content sections
		this.selCommunities = this.defaultCommunities;

		// Wait for Vue to update the DOM based on the new `popComms` data
		this.$nextTick(() => {
			// Loop through the default communities to programmatically check their boxes
			for (let j = 0; j < this.defaultCommunities.length; j++) {

				const community = this.defaultCommunities[j];

				// Defensive check 1: Ensure the community data itself is a valid array
				if (community && Array.isArray(community) && community.length > 0) {
					const communityID = community[0];
					const refName = communityID + 'chk';

					const refElement = this.$refs[refName];

					// Defensive check 2: Ensure the $ref was found and is a valid array/element
					if (refElement && refElement[0]) {
						refElement[0].checked = true;
					}
				}
			}
		});
