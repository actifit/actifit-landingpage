    ...
    <section id="recent-activity" class="py-5">
      <div class="container-fluid">
        ...
        <div v-if="!isLoadingReports && fullReports.length > 0">
          <client-only>
            <carousel 
              :perPageCustom="[[0, 1], [576, 2], [992, 4], [1200, 5]]" 
              :paginationEnabled="false"
              :navigationEnabled="true"
              :navigationNextLabel="'<i class=\'fas fa-chevron-right\'></i>'"
              :navigationPrevLabel="'<i class=\'fas fa-chevron-left\'></i>'"
            >
              <slide v-for="(report, index) in fullReports" :key="'activity-' + index">
                <div class="activity-post-card-carousel-wrapper">
                  <div class="activity-post-card">
                    <a :href="report.url" target="_blank" class="activity-card-link">
                      ...
                      <div class="post-stats" v-if="report.json_metadata && report.json_metadata.step_count">
                        ...
                        <span>{{ numberFormat(report.json_metadata.step_count, 0) }} {{ $t('Steps') }}</span>
                      </div>
                    </a>
                  </div>
                </div>
...
export default {
  ...
  methods: {
    enrichDelegatorData(delegators) {
      ...
      const a = delegators.map(d => {
        const placeholderEarnings = (d.steem_power / 1000).toFixed(2);
        return {
          ...d,
          daily_earnings: placeholderEarnings
        };
      });
      this.enrichedDelegators = a;
