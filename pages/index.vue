              ...
              <div class="custom-scroller" ref="activityScroller">
                  <div class="custom-scroller-inner">
                      <div v-for="(report, index) in fullReports" :key="'activity-' + index" class="activity-card-scroller-item">
                          <div class="activity-post-card h-100">
...
export default {
  ...
  methods: {
    ...
    toRelativeTime(dateString) {
      ...
      // Append 'Z' to ensure the date is parsed in UTC, as Hive dates are in UTC
    ...
    extractPostImage(post) {
        ...
        // As a fallback, find the first image URL in the post body
        if (post.body) {
...
<style>
/* --- MODIFICATION START --- */
/* This rule forces the page to always reserve space for the scrollbar, preventing a width change when a modal opens. */
body.home-page-active {
  overflow-x: hidden;
  overflow-y: scroll; 
}

/* This rule overrides Bootstrap's default behavior for modals, preventing it from hiding overflow and adding padding, which caused the conflict. */
body.modal-open {
  overflow: visible;
