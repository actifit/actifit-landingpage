    ...
    <div class="modal-dialog modal-lg" role="document">
      <div class="modal-content" v-if="pendingRewards.pendingRewards">
        ...
        <div class="p-2 m-3 ">
          ...
          <br /><br />
          <input type="checkbox" name="hidePendingRewards" id="hidePendingRewards" ref="hidePendingRewards"
            @change="updatePopupStoredStatus($event)" />{{ $t('hide_pending_rewards_popup') }}
          <div class="text-center">
            <button type="button" class="btn btn-lg btn-brand w-25" data-dismiss="modal" aria-label="Close">
              &nbsp;{{ $t('Close') }}
            </button>
            <a v-if="username" :href="'./activity/' + username.account.name" class="btn btn-lg btn-brand w-25">{{
              $t('My_Activity') }}</a>
          </div>
        </div>
      </div>
  ...
  methods: {
    ...
    fetchBlurtPrice() {
      ...
      fetch('https://api.coingecko.com/api/v3/simple/price?ids=blurt&vs_currencies=usd').then(
        res => {
          res.json().then(json => this.setBlurtPrice(json.blurt.usd)).catch(e => console.log(e))
