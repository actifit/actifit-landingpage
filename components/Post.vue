<template>
  ...
  <div>
    <div :class="[isOnlyPost ? 'card post single' : { 'card-pinned': isPostPinned, 'card post': isStandardPost }]">
      ...
      <div class="post-body" :style="'background-image:url('+$fetchPostImage(meta)+') ;'" :class="isMinimal?'post-body-minimal':''">

        <!--<div class="image-overlay"></div> --><!-- Semi-transparent overlay -->
        ...
        <div class="row">
          <div class="col-12">
            <a href="#" class="text-brand" @click="post.pstId = pstId; $store.commit('setActivePost', post)"
              ...
              <img v-if="!isMinimal" :src="$fetchPostImage(meta)" :alt="post.title"
                class="post-image">
            </a>
          </div>
        </div>
        ...
        <div class="row details mt-2">
          <div class="col-12">
            ...
            <span v-if="hasBeneficiaries() && !isMinimal" :title="beneficiariesDisplay()" @click="toggleTooltip()"
...
export default {
  props: ['userPosts', 'post', 'displayUsername', 'pstId', 'explorePost', 'minimal'],
  computed: {
