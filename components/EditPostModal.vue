      ...
      <div class="modal-content" v-if="editPost">
        <div class="modal-header">
          <h5 v-if="editPost.isNewPost" class="modal-title" id="exampleModalLabel">
            {{ $t('Blog_on_hive') }}
            <a v-if="user" :href="'/' + user.account.name + '/blog/new'" class="btn btn-brand border"><i
                class="fa-regular fa-maximize fas"></i></a>
          </h5>
          <h5 v-else class="modal-title" id="exampleModalLabel">{{ $t('Editing') }} <a
              :href="'/' + editPost.author + '/' + editPost.permlink">{{ title }}</a></h5>
        ...
        <div class="modal-body">
          ...
          <div class="form-group acti-shadow extra-container">
            ...
            <!-- also select community to post to, only in case of new post -->
            <div class="form-group d-flex align-items-center p-1">
              <select id="targetCommunity" ref="targetCommunity" class="form-control targetCommunity"
                v-show="editPost.isNewPost === true">
