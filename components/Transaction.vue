  ...
  <div class="card p-3 my-2">
    <div class="d-flex flex-row justify-content-between">
      ...
      <div>
		...
		<a :href="'https://busy.org' + '/@' + transaction.user + '/' + transaction.url" target="_blank" v-if="transaction.reward_activity == 'Full AFIT Payout'">{{ $t('Activity_Details') }}</a>
        <a :href="'https://busy.org' + transaction.url" target="_blank" v-else-if="transaction.url">{{ $t('Activity_Details') }}</a>
      </div>
    </div>
