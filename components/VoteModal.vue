			...
			<div class="modal-content border border-danger" v-if="postToVote">
				...
				<div class="modal-body text-center">
					...
					<ul class="pagination justify-content-center mt-2" v-if="this.user">
						<li :class="{ 'page-item': true, disabled: voteWeight === -100 }"><a
								class="page-link vote-controls" href="#" @click.prevent="setVoteWeight(-100)"><i
									class="far fa-flag text-danger"></i></a></li>
						<li :class="{ 'page-item': true, disabled: voteWeight === -100 }"><a
								class="page-link vote-controls text-danger" href="#"
								@click.prevent="changeVoteWeight(-10)">-10</a></li>
						<li :class="{ 'page-item': true, disabled: voteWeight === -100 }"><a
								class="page-link vote-controls text-danger" href="#"
								@click.prevent="changeVoteWeight(-1)">-1</a></li>
						<li class="page-item vote-weight">
							<input id="votableWeight" type="number" value="100" class="form-control"
								v-model="voteWeight" @change="customVoteWeight" />
						</li>
						<li :class="{ 'page-item': true, disabled: voteWeight === 100 }"><a
								class="page-link vote-controls text-success" href="#"
								@click.prevent="changeVoteWeight(1)">+1</a></li>
						<li :class="{ 'page-item': true, disabled: voteWeight === 100 }"><a
								class="page-link vote-controls text-success" href="#"
								@click.prevent="changeVoteWeight(10)">+10</a></li>
						<li :class="{ 'page-item': true, disabled: voteWeight === 100 }"><a
								class="page-link vote-controls" href="#" @click.prevent="setVoteWeight(100)"><i
									class="far fa-thumbs-up text-success"></i></a></li>
					</ul>
					<div v-if="this.user">
						<span class="btn btn-brand mb-1" :title="$t('save_vote_default_settings')"
							v-on:click="saveSettings"><i class="fa-gear fas"
								:style="settings_set ? 'color:green' : 'color:white'"></i><i
								class="fas fa-spin fa-spinner text-white" v-if="save_progress"></i></span>
						<span class="btn btn-brand mb-1" :title="$t('restore_default')" v-if="settings_set"
							v-on:click="restoreDefault"><i class="fa-solid fa-arrows-rotate"></i></span>
					</div>
					<div class="text-center"><span>{{ $t('Your_Vote_Value') }}:</span><span>{{ vote_value_usd }}</span>
					</div>
