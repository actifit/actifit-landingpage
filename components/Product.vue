  <template>
    ...
    <div class="detail-item boost-item" 
      ...
      <span class="boost-value">
        ...
        <!-- Status Indicators -->
        <div class="status-indicators" v-if="grabConsumableItem()">
          <div class="status-item">
            <span class="status-label">{{ $t('Status') }}</span>
            <span class="status-value" :class="grabConsumableItem().status == 'active' ? 'active' : 'inactive'">
          ...
          <div class="status-item" v-if="prodHasFriendBenefic()">
            <span class="status-label">{{ $t('Benef_friend') }}</span>
            <span class="status-value" v-if="grabConsumableItem().status == 'active'">
          ...
          <div class="status-item">
            <span class="status-label">{{ $t('Remaining_boost') }}</span>
            <span class="status-value">{{ grabConsumableItem().span - grabConsumableItem().consumed }} {{ $t('Activity_Reports') }}</span>
          </div>
        </div>
      </div>
    ...
    <div class="pricing-container">
      ...
      <template v-if="item_price_extra <= 0">
        <div class="single-price-row">
          ...
          <button class="btn-buy" @click="product.type == 'real' ? prepareCheckout() : buyNow()">
            {{ $t('Buy_now') }}
          </button>
        </div>
      </template>
      ...
      <template v-else>
        <div class="dual-prices-grid">
          ...
          <div class="price-column">
            ...
            <button class="btn-buy-hive" @click="buyNowHive">
              {{ $t('Buy_with') }} HIVE
            </button>
          </div>
        </div>
      ...
      <!-- Expandable Section -->
      <div class="expandable-section" :class="{'is-expanded': prodDispStatus === 'open'}">
        <div class="expand-toggle" @click="switchArrowStatus">
          {{ prodDispStatus === 'open' ? $t('Show Less') : $t('Show More') }}
          <i class="fas" :class="prodDispStatus === 'open' ? 'fa-chevron-up' : 'fa-chevron-down'"></i>
        </div>
        
        <div class="expanded-content" v-if="prodDispStatus === 'open'">
          <!-- Product Description -->
        ...
        <div v-if="lightboxActive" class="lightbox-modal" @click.self="closeLightbox">
          ...
          <div class="additional-actions" v-if="user && product.type == 'ingame' && allReqtsFilled">
            <button class="btn-add-cart" 
              ...
              <i class="fas fa-cart-plus"></i> {{ $t('Add_Cart') }}
            </button>
            <button class="btn-add-cart" 
                    @click.prevent="removeCart()" 
                    v-else>
              <i class="fas fa-cart-arrow-down"></i> {{ $t('Remove_Cart') }}
            </button>
            <button class="btn-checkout" data-toggle="modal" data-target="#cartModal">
              <i class="fas fa-shopping-bag"></i> {{ $t('Checkout') }}
            </button>
          </div>
        </div>
      </div>
      ...
      <!-- Checkout Form -->
      <div class="checkout-form" v-if="checkout_product">
        <h4>{{ $t('shipping_details') }}</h4>
        <div class="form-group">
          <input type="text" name="buyer_name" id="buyer_name" ref="buyer_name" 
                :placeholder="$t('buyer_name')" class="form-control">
        </div>
        <div class="form-group">
          <input type="text" name="buyer_phone" id="buyer_phone" ref="buyer_phone" 
                :placeholder="$t('buyer_phone')" class="form-control">
        </div>
        <div class="form-group">
          <input type="text" name="buyer_address" id="buyer_address" ref="buyer_address" 
                :placeholder="$t('buyer_address')" class="form-control">
        </div>
        <div class="form-group">
          <input type="text" name="buyer_address2" id="buyer_address2" ref="buyer_address2"
                :placeholder="$t('buyer_address2')" class="form-control">
        </div>
        <div class="form-group">
          <select v-model="buyer_country" class="form-control">
            <option value="" disabled selected>{{ $t('buyer_country') }}</option>
            <option v-for="country_inst in product.countries" :value="country_inst" :key="country_inst">
              {{ country_inst }}
            </option>
          </select>
        </div>
        <div class="form-group">
          <input type="text" name="buyer_state" id="buyer_state" ref="buyer_state" 
                class="form-control" :placeholder="$t('buyer_state')">
        </div>
        <div class="form-group">
          <input type="text" name="buyer_city" id="buyer_city" ref="buyer_city" 
                class="form-control" :placeholder="$t('buyer_city')">
        </div>
        <div class="form-group">
          <input type="text" name="buyer_zip" id="buyer_zip" ref="buyer_zip" 
                class="form-control" :placeholder="$t('buyer_zip')">
        </div>
        
        <div class="form-group" v-if="item_price_extra > 0 && !isKeychainActive && !isHiveauthActive">
          <label>{{ $t('Active_Key') }} *</label>
          <input type="password" id="rl-active-key" name="rl-active-key" 
                ref="rl-active-key" class="form-control" v-model="userRlActvKey">
        </div>
        
        <div class="form-notice" v-html="$t('real_prod_notice')"></div>
        
        <div class="form-actions">
          <button @click="checkout_product = false" class="btn btn-secondary">
            {{ $t('Cancel') }}
          </button>
          <button @click="proceedBuyReal()" class="btn btn-primary">
            {{ $t('Proceed') }}
          </button>
        </div>
        
        <div v-if="buyInProgress && errorProceed == ''" class="loading-indicator">
          <i class="fas fa-spinner fa-spin"></i> {{ $t('Processing') }}...
        </div>
        <div v-else-if="errorProceed" class="error-message">
          {{ errorProceed }}
        </div>
      </div>
...
getPrice() {
  if (this.product && this.afitPrice) {
    ...
    for (let i = 0; i < price_options_count; i++) {
      ...
      if (this.item_currency == 'USD') {
        ...
        // ADD THIS BLOCK FOR NON-USD PRODUCTS
        if (this.product.type !== 'real') {
          this.item_price_extra = Number(
          ...
          // For real products in AFIT, keep single price
  ...
  .detail-row.boosts {
    ...
    background: rgba(231, 76, 60, 0.1); /* Same as availability */
    padding: 8px 12px;
    border-radius: 8px;
    margin-bottom: 15px;
  }

  /* Style each boost item like availability items */
  .detail-item.boost-item {
    display: flex;
    align-items: center;
    gap: 8px;
    font-size: 0.9rem;
  ...
  .boost-icon.friend {
    color: #e74c3c; /* Red for friend boosts */
  }
